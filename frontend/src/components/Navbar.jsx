    ...
    <nav className="bg-white/95 backdrop-blur-md shadow-lg border-b border-emerald-200 sticky top-0 z-50 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* Logo and Brand with enhanced animations */}
          <div className="flex items-center space-x-8">
            ...
            >
              ...
              <span className="text-xl font-bold text-gray-800 transition-colors duration-300 hover:text-emerald-600">Rental Pro</span>
            </Link>
            
            {/* Navigation Menu - Desktop with smooth animations */}
              ...
              {navigation.map((item, index) => {
                ...
                return (
                  <Link
                    key={item.name}
                    to={item.href}
                    className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 magnetic-btn cursor-pointer hover-lift ${
                      isActive
                  ...
                  >
                    <span className={`transition-transform duration-300 ${hoveredItem === item.name ? 'scale-110' : ''}`}>
                      {getIcon(item.icon)}
                    </span>
          ...
          {/* Right Side - Search, Notifications, User with enhanced animations */}
          <div className="flex items-center space-x-4">
            
            {/* Search Bar - Desktop with smooth focus */}
            <div className="hidden lg:block">
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none transition-colors duration-300 group-focus-within:text-emerald-500">
                  <svg className="h-4 w-4 text-gray-400 transition-colors duration-300 group-focus-within:text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
